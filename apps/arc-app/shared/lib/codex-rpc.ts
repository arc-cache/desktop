...
export class CodexRpcClient {
  ...
  constructor(private proc: ChildProcess, options: CodexRpcClientOptions) {
    ...
    this.processLabel = options.processLabel ?? "Codex";
    ...
    proc.on("exit", (code, signal) => {
      ...
      for (const [id, pending] of this.pendingRequests) {
        ...
        pending.reject(new Error(`${this.processLabel} process exited (code=${code}, signal=${signal}) while waiting for ${pending.method}`));
  ...
  async request<T = unknown>(method: string, params?: unknown, timeoutMs = DEFAULT_TIMEOUT_MS): Promise<T> {
    if (this.destroyed) throw new Error(`${this.processLabel} RPC client is destroyed`);

    const id = this.nextId++;
    return new Promise<T>((resolve, reject) => {
      const timer = setTimeout(() => {
        this.pendingRequests.delete(id);
        reject(new Error(`${this.processLabel} RPC timeout: ${method} (id=${id}) after ${timeoutMs}ms`));
      }, timeoutMs);
