...
function readBootstrapArgument(name: string): string | null {
  const prefix = `--${name}=`;
...
contextBridge.exposeInMainWorld("claude", {
  // ARC bootstrap values are passed as renderer arguments so packaged launches
  // do not depend on cross-process environment mutation timing.
  arcInitialProjectPath: readBootstrapArgument("arc-initial-project") ?? process.env.ARC_INITIAL_PROJECT ?? null,
  arcPanelUrl: readBootstrapArgument("arc-panel-url") ?? process.env.ARC_PANEL_URL ?? null,
  ...
  projects: {
    ...
    resetCache: (projectId: string) => ipcRenderer.invoke("projects:reset-cache", projectId),
  ...
  codex: {
    ...
    setPermissionMode: (sessionId: string, approvalPolicy?: string, sandbox?: "read-only" | "workspace-write" | "danger-full-access") =>
      ipcRenderer.invoke("codex:set-permission-mode", { sessionId, approvalPolicy: approvalPolicy ?? null, sandbox: sandbox ?? null }),
  ...
  copilot: {
    log: (label: string, data: unknown) => ipcRenderer.send("copilot:log", label, data),
    start: (options: { cwd: string; model?: string; approvalPolicy?: string; sandbox?: "read-only" | "workspace-write" | "danger-full-access" }) =>
      ipcRenderer.invoke("copilot:start", options),
    send: (sessionId: string, text: string, images?: Array<{ type: "image"; url: string } | { type: "localImage"; path: string }>, effort?: string) =>
      ipcRenderer.invoke("copilot:send", { sessionId, text, images, effort }),
    stop: (sessionId: string) => ipcRenderer.invoke("copilot:stop", sessionId),
    interrupt: (sessionId: string) => ipcRenderer.invoke("copilot:interrupt", sessionId),
    respondApproval: (sessionId: string, rpcId: string | number, decision: string, acceptSettings?: unknown) =>
      ipcRenderer.invoke("copilot:approval_response", { sessionId, rpcId, decision, acceptSettings }),
    respondUserInput: (sessionId: string, rpcId: string | number, answers: Record<string, { answers: string[] }>) =>
      ipcRenderer.invoke("copilot:user_input_response", { sessionId, rpcId, answers }),
    respondServerRequestError: (sessionId: string, rpcId: string | number, code: number, message: string) =>
      ipcRenderer.invoke("copilot:server_request_error", { sessionId, rpcId, code, message }),
    compact: (sessionId: string) => ipcRenderer.invoke("copilot:compact", sessionId),
    listSkills: (sessionId: string) => ipcRenderer.invoke("copilot:list-skills", sessionId),
    listApps: (sessionId: string) => ipcRenderer.invoke("copilot:list-apps", sessionId),
    listModels: () => ipcRenderer.invoke("copilot:list-models"),
    authStatus: () => ipcRenderer.invoke("copilot:auth-status"),
    login: (sessionId: string, type: "apiKey" | "chatgpt", apiKey?: string) =>
      ipcRenderer.invoke("copilot:login", { sessionId, type, apiKey }),
    resume: (options: { cwd: string; threadId: string; model?: string; approvalPolicy?: string; sandbox?: "read-only" | "workspace-write" | "danger-full-access" }) =>
      ipcRenderer.invoke("copilot:resume", options),
    setModel: (sessionId: string, model: string) =>
      ipcRenderer.invoke("copilot:set-model", { sessionId, model }),
    setPermissionMode: (sessionId: string, approvalPolicy?: string, sandbox?: "read-only" | "workspace-write" | "danger-full-access") =>
      ipcRenderer.invoke("copilot:set-permission-mode", { sessionId, approvalPolicy: approvalPolicy ?? null, sandbox: sandbox ?? null }),
    version: () => ipcRenderer.invoke("copilot:version"),
    binaryStatus: () => ipcRenderer.invoke("copilot:binary-status"),
    onEvent: (callback: (data: unknown) => void) => {
      const listener = (_event: IpcRendererEvent, data: unknown) => callback(data);
    ...
    onApprovalRequest: (callback: (data: unknown) => void) => {
      const listener = (_event: IpcRendererEvent, data: unknown) => callback(data);
      ipcRenderer.on("copilot:approval_request", listener);
      return () => ipcRenderer.removeListener("copilot:approval_request", listener);
    },
    onExit: (callback: (data: unknown) => void) => {
      const listener = (_event: IpcRendererEvent, data: unknown) => callback(data);
      ipcRenderer.on("copilot:exit", listener);
      return () => ipcRenderer.removeListener("copilot:exit", listener);
    },
  },
  mcp: {
  ...
  team: {
    ...
    sendEmailSignIn: (email: string) => ipcRenderer.invoke("team:send-email-sign-in", email),
    verifyEmailOtp: (params: { email: string; token: string }) => ipcRenderer.invoke("team:verify-email-otp", params),
    signInWithGitHub: () => ipcRenderer.invoke("team:sign-in-github"),
    signOut: () => ipcRenderer.invoke("team:sign-out"),
    forgetState: () => ipcRenderer.invoke("team:forget-state"),
    callbackUrls: () => ipcRenderer.invoke("team:callback-urls"),
    consumeCallbackEvent: () => ipcRenderer.invoke("team:consume-callback-event"),
    createWorkspace: (name: string) => ipcRenderer.invoke("team:create-workspace", name),
    setActiveWorkspace: (workspaceId: string | null) => ipcRenderer.invoke("team:set-active-workspace", workspaceId),
    listLocalCapsules: (localWorkspacePath: string) => ipcRenderer.invoke("team:list-local-capsules", localWorkspacePath),
    listCapsules: (workspaceId: string) => ipcRenderer.invoke("team:list-capsules", workspaceId),
    shareCapsule: (params: unknown) => ipcRenderer.invoke("team:share-capsule", params),
    shareMarkedCapsules: (params: unknown) => ipcRenderer.invoke("team:share-marked-capsules", params),
    pullCapsules: (params: unknown) => ipcRenderer.invoke("team:pull-capsules", params),
    syncWorkspace: (params: unknown) => ipcRenderer.invoke("team:sync-workspace", params),
    deleteCapsule: (params: unknown) => ipcRenderer.invoke("team:delete-capsule", params),
    getProjectAutoShare: (params: unknown) => ipcRenderer.invoke("team:get-project-auto-share", params),
    setProjectAutoShare: (params: unknown) => ipcRenderer.invoke("team:set-project-auto-share", params),
    listMembers: (workspaceId: string) => ipcRenderer.invoke("team:list-members", workspaceId),
    listPendingInvites: (workspaceId: string) => ipcRenderer.invoke("team:list-pending-invites", workspaceId),
    listJoinableInvites: () => ipcRenderer.invoke("team:list-joinable-invites"),
    wrapPendingKeys: (workspaceId: string) => ipcRenderer.invoke("team:wrap-pending-keys", workspaceId),
    initializeWorkspaceAccess: (workspaceId: string) => ipcRenderer.invoke("team:initialize-workspace-access", workspaceId),
    setMemberRole: (params: unknown) => ipcRenderer.invoke("team:set-member-role", params),
    transferOwnership: (params: unknown) => ipcRenderer.invoke("team:transfer-ownership", params),
    removeMember: (params: unknown) => ipcRenderer.invoke("team:remove-member", params),
    createInvite: (params: unknown) => ipcRenderer.invoke("team:create-invite", params),
    acceptInvite: (token: string) => ipcRenderer.invoke("team:accept-invite", token),
    acceptInviteById: (inviteId: string) => ipcRenderer.invoke("team:accept-invite-by-id", inviteId),
  },
  jira: {
