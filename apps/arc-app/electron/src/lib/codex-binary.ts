...
import { childProcessEnv } from "./electron-node";
...
function resolveCodexPathSync(): string {
  ...
  try {
    ...
    const resolved = execFileSync(cmd, ["codex"], { encoding: "utf-8", timeout: 5000, env: childProcessEnv() }).trim();
...
async function downloadCodexBinary(): Promise<string> {
  ...
  try {
    ...
    // Extract
    execFileSync("tar", ["xzf", tgzPath], { cwd: tmpDir, timeout: 30000, env: childProcessEnv() });
