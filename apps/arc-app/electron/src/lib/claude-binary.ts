...
import { childProcessEnv, electronNodeEnv } from "./electron-node";
...
function resolveFromPathLookup(): ClaudeBinaryResolution | null {
  try {
    ...
    const output = execFileSync(cmd, ["claude"], { encoding: "utf-8", timeout: 5000, env: childProcessEnv() });
...
function readClaudeVersion(binaryPath: string): string | null {
  ...
  const output = execFileSync(command, args, {
    encoding: "utf-8",
    timeout: 10000,
    env: command === process.execPath ? electronNodeEnv() : childProcessEnv(),
  }).trim();
  return output || null;
}
