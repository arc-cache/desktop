...
import { captureException } from "./local-events";

export { extractErrorMessage } from "@shared/lib/error-utils";
import { extractErrorMessage } from "@shared/lib/error-utils";

/**
 * Log an error and return the extracted message in one call.
 *
 * Replaces the common `log(label, extractErrorMessage(err))` pattern with
 * a single call. The local event sink is intentionally non-networked.
 *
