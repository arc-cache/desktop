...
describe("claude binary resolution", () => {
  ...
  it("runs sdk fallback script version checks through Electron Node mode", async () => {
    mockExecFileSync.mockImplementation((command: string, args: string[], options: { env?: NodeJS.ProcessEnv }) => {
