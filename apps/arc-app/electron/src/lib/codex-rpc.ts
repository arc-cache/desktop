 ...
 *
 * Electron consumers can continue to use `new CodexRpcClient(proc)` — the factory
...
export class CodexRpcClient extends SharedCodexRpcClient {
  constructor(proc: ChildProcess, options: { processLabel?: string } = {}) {
    super(proc, { log, reportError, processLabel: options.processLabel });
  }
}
