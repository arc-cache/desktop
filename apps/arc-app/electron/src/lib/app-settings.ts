...
function stripLegacyAnalyticsSettings(value: Partial<AppSettings> & Record<string, unknown>): Partial<AppSettings> {
  const {
    analyticsEnabled: _analyticsEnabled,
    analyticsUserId: _analyticsUserId,
    analyticsLastDailyActiveDate: _analyticsLastDailyActiveDate,
    ...settings
  } = value;
...
export function getAppSettings(): AppSettings {
  ...
  try {
    ...
    const parsed = stripLegacyAnalyticsSettings(JSON.parse(raw) as Partial<AppSettings> & Record<string, unknown>);
...
export function setAppSettings(patch: Partial<AppSettings>): AppSettings {
  ...
  const next = stripLegacyAnalyticsSettings({ ...current, ...patch }) as AppSettings;
  cached = next;
