...
import { beginArcTurn, recordClaudeSdkEvent } from "../lib/arc-host";
...
function fileCheckpointOptions(): Record<string, unknown> {
  return {
    ...
    env: childProcessEnv({ ...clientAppEnv(), CLAUDE_CODE_ENABLE_SDK_FILE_CHECKPOINTING: "1" }),
  };
}
...
function applyPermissionModeOptions(
  ...
  // ARC exposes "Allow All" as a runtime mode switch. The SDK only lets a
...
function startEventLoop(
  ...
  // Maps tool_use_id to tool name, populated from assistant events so result
  // events can reference the originating tool.
  ...
  (async () => {
    try {
      for await (const message of queryHandle) {
        ...
        recordClaudeSdkEvent(sessionId, msgObj, (notice) => {
          safeSend(getMainWindow, "claude:event", {
            ...notice,
            type: "arc_memory",
...
function promptFromSdkContent(content: unknown): string {
...
function replacePromptInSdkContent(content: unknown, nextPrompt: string): unknown {
  if (typeof content === "string") return nextPrompt;
  if (!Array.isArray(content)) return nextPrompt;

  let replaced = false;
  const next = content.map((item) => {
    if (replaced || !item || typeof item !== "object" || Array.isArray(item)) return item;
    const record = item as Record<string, unknown>;
    if (record.type !== "text") return item;
    replaced = true;
    return { ...record, text: nextPrompt };
  });
  return replaced ? next : [{ type: "text", text: nextPrompt }, ...content];
}
...
export function register(getMainWindow: () => BrowserWindow | null): void {
  ...
  ipcMain.handle("claude:send", async (_event, { sessionId, message }: { sessionId: string; message: { message: { content: unknown } } }) => {
    ...
    const prompt = promptFromSdkContent(originalContent);
    const arc = await beginArcTurn({
      engine: "claude",
      sessionId,
      cwd: session.startOptions?.cwd || process.cwd(),
      prompt,
    });
    const content = arc.injected
      ? replacePromptInSdkContent(originalContent, arc.prompt)
      : originalContent;
    log("SEND", `session=${sessionId.slice(0, 8)} content=${JSON.stringify(message).slice(0, 500)} arc=${arc.injected ? "injected" : "none"}`);
