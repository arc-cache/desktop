...
import { isFatalProviderError, markUtilityProviderUnavailable } from "../lib/provider-health";
...
import { SUPPORTED_SERVER_REQUESTS, isSupportedServerRequestMethod, pickModelId, sandboxModeToCodexPolicy } from "@shared/lib/codex-helpers";
...
function getAppServerClientInfo(): { name: string; title: string; version: string } {
  const clientName = getAppSetting("codexClientName") || "ARC";
  return {
    name: clientName,
    title: clientName,
...
function setupCodexHandlers(
  ...
  rpc.onNotification = (msg) => {
    ...
    recordCodexLikeNotification("codex", internalId, notification, (notice) => {
  ...
  rpc.onExit = (code, signal) => {
    ...
    void finishArcTurn("codex", internalId, "failed").then((notice) => {
      if (!notice) return;
      safeSend(getMainWindow, "codex:event", {
        _sessionId: internalId,
        method: "arc/memory",
...
export function register(getMainWindow: () => BrowserWindow | null): void {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        const sandboxPolicy = sandboxModeToCodexPolicy(session.sandbox);
        ...
        if (arcTurnStarted && !session.activeTurnId) {
          await finishArcTurn("codex", data.sessionId, "failed").catch(() => undefined);
        }
        if (isFatalProviderError("codex", errMsg)) {
          markUtilityProviderUnavailable("codex", errMsg);
        }
        return { error: errMsg };
      }
    },
  ...
  ipcMain.handle(
    ...
    async (_, data: { sessionId: string; approvalPolicy?: string | null; sandbox?: string | null }) => {
      const session = codexSessions.get(data.sessionId);
      if (!session) return { error: "Session not found" };
