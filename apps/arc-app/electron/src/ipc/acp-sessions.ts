...
interface ACPSessionEntry {
  ...
  acpSessionId?: string;
  internalId: string;
  eventProperties: AcpEventProperties;
  eventCounter: number;
...
function buildAcpEventProperties(agent: InstalledAgent): AcpEventProperties {
...
async function createAcpConnection(
  ...
  const proc = spawn(agentDef.binary, agentDef.args ?? [], {
    ...
    env: childProcessEnv(agentDef.env),
...
export function register(getMainWindow: () => BrowserWindow | null): void {
  ...
  ipcMain.handle("acp:start", async (_event, options: { agentId: string; cwd: string; mcpServers?: McpServerInput[] }) => {
    ...
    const eventProperties = buildAcpEventProperties(agentDef);
    try {
      ...
      void captureEvent("session_created", { engine: "acp", ...eventProperties });
      ...
      const msg = reportError("ACP_SPAWN", err, { engine: "acp", ...eventProperties });
      return { error: msg };
    }
  });
  ...
  ipcMain.handle("acp:revive-session", async (_event, options: {
    ...
    try {
      ...
      if (supportsLoadSession && options.agentSessionId) {
        ...
        const entry: ACPSessionEntry = { process: proc, connection, acpSessionId: options.agentSessionId, internalId, eventProperties, eventCounter: 0, pendingPermissions, cwd: options.cwd, supportsLoadSession, agentName: agentDef.name, authMethods, isReloading: true };
        acpSessions.set(internalId, entry);
        ...
        const entry: ACPSessionEntry = { process: proc, connection, acpSessionId, internalId, eventProperties, eventCounter: 0, pendingPermissions, cwd: options.cwd, supportsLoadSession, agentName: agentDef.name, authMethods, isReloading: false };
        acpSessions.set(internalId, entry);
      ...
      void captureEvent("session_revived", { engine: "acp", success: true, ...eventProperties });
      ...
      const msg = reportError("ACP_REVIVE", err, { engine: "acp", ...eventProperties });
      return { error: msg };
    }
  });
