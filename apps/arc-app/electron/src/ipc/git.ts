...
const WORKTREE_SETUP_FILES = [".arc/worktree.json", ".harnss/worktree.json"] as const;
...
function shellExec(command: string, cwd: string): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    execFile(shell, [flag, command], { cwd, timeout: 120_000, env: childProcessEnv() }, (err, stdout, stderr) => {
...
/**
 * Run post-creation setup commands from .arc/worktree.json.
 * Falls back to .harnss/worktree.json for existing workspaces.
...
async function runWorktreeSetup(
  ...
  const setupFile = WORKTREE_SETUP_FILES.find((file) => fs.existsSync(path.join(sourceRepoPath, file)));
  if (!setupFile) return [];
  const configPath = path.join(sourceRepoPath, setupFile);

  let config: { "setup-worktree"?: string[] };
  try {
    config = JSON.parse(fs.readFileSync(configPath, "utf-8"));
  } catch (err) {
    log("GIT_WORKTREE_SETUP", `Failed to parse ${setupFile}: ${err}`);
    return [];
  }
...
export function register(): void {
  ...
  ipcMain.handle("git:create-worktree", async (_event, { cwd, path: worktreePath, branch, fromRef }: { cwd: string; path: string; branch: string; fromRef?: string }) => {
    try {
      ...
      // Run post-creation setup from .arc/worktree.json (non-blocking)
