...
const DEFAULT_TOOL_ORDER: ToolId[] = ["terminal", "git", "browser", "files", "project-files", "mcp", "memory", "sharing"];
...
function migrateFromLegacyLocalStorage(): { global: GlobalSettingsState; projects: Record<string, ProjectSettings> } {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    const perProjectSuffixes = [
      "-model-claude", "-model-acp", "-model-codex", "-model-copilot", "-model",
...
function readLegacyModelForEngine(pid: string, engine: EngineId): string {
  ...
  if (engine === "codex" || engine === "copilot") {
    return isCodexLikeModel(legacyValue) ? legacyValue : DEFAULT_ENGINE_MODELS[engine];
  }
...
function readLegacyProjectSettings(pid: string): ProjectSettings {
  return {
    modelsByEngine: {
      ...
      copilot: readLegacyModelForEngine(pid, "copilot"),
    },
...
function validateToolList(stored: ToolId[] | undefined): ToolId[] {
  return (stored ?? []).filter((id) => VALID_TOOL_IDS.has(id));
}

function sanitizeProjectSettings(project: ProjectSettings): ProjectSettings {
  return {
    ...project,
    activeTools: validateToolList(project.activeTools),
    toolOrder: validateToolOrder(project.toolOrder),
    suppressedPanels: validateToolList(project.suppressedPanels),
    bottomTools: validateToolList(project.bottomTools),
  };
}

function sanitizeProjects(projects: Record<string, ProjectSettings>): Record<string, ProjectSettings> {
  const next: Record<string, ProjectSettings> = {};
  for (const [projectId, project] of Object.entries(projects)) {
    next[projectId] = sanitizeProjectSettings(project);
  }
  return next;
}
    ...
    }),
    {
      ...
      merge: (persisted, current) => {
        ...
        return {
          ...
          projects: sanitizeProjects(incoming.projects ?? current.projects),
        };
      },
    },
  ),
