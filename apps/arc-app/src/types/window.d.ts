...
type ProjectCreateResult = Project | { error: string } | null;
...
declare global {
  ...
  interface Window {
    claude: {
      arcInitialProjectPath?: string | null;
      arcPanelUrl?: string | null;
      ...
      projects: {
        ...
        create: (spaceId?: string) => Promise<ProjectCreateResult>;
        createDev: (name: string, spaceId?: string) => Promise<ProjectCreateResult>;
        delete: (projectId: string) => Promise<IpcResult>;
        rename: (projectId: string, name: string) => Promise<IpcResult>;
        resetCache: (projectId: string) => Promise<IpcResult & { cacheDir?: string; removed?: boolean }>;
      ...
      copilot: {
        ...
        listModels: () => Promise<{ models: CodexModel[]; error?: string }>;
        authStatus: () => Promise<CopilotAuthStatusResponse>;
        login: (sessionId: string, type: "apiKey" | "chatgpt", apiKey?: string) => Promise<unknown>;
        resume: (options: { cwd: string; threadId: string; model?: string; approvalPolicy?: string; sandbox?: "read-only" | "workspace-write" | "danger-full-access" }) =>
          Promise<{ sessionId?: string; threadId?: string; error?: string }>;
        setModel: (sessionId: string, model: string) => Promise<{ error?: string }>;
      ...
      team: {
        status: () => Promise<ArcTeamStatus | { error: string }>;
        sendEmailSignIn: (email: string) => Promise<ArcTeamEmailSignInResult | { error: string }>;
        verifyEmailOtp: (params: { email: string; token: string }) => Promise<ArcTeamStatus | { error: string }>;
        signInWithGitHub: () => Promise<ArcTeamStatus | { error: string }>;
        signOut: () => Promise<ArcTeamStatus | { error: string }>;
        forgetState: () => Promise<ArcTeamStatus | { error: string }>;
        callbackUrls: () => Promise<ArcTeamCallbackUrls | { error: string }>;
        consumeCallbackEvent: () => Promise<ArcTeamCallbackEvent | null | { error: string }>;
        createWorkspace: (name: string) => Promise<ArcTeamStatus | { error: string }>;
        setActiveWorkspace: (workspaceId: string | null) => Promise<ArcTeamStatus | { error: string }>;
        listLocalCapsules: (localWorkspacePath: string) => Promise<ArcLocalCapsuleSummary[] | { error: string }>;
        listCapsules: (workspaceId: string) => Promise<ArcTeamCapsuleMeta[] | { error: string }>;
        shareCapsule: (params: {
          workspaceId: string;
          localWorkspacePath: string;
          capsuleId: string;
          allowWarnings?: boolean;
        }) => Promise<ArcTeamShareResult | { error: string }>;
        shareMarkedCapsules: (params: {
          workspaceId: string;
          localWorkspacePath: string;
        }) => Promise<ArcTeamShareResult[] | { error: string }>;
        pullCapsules: (params: {
          workspaceId: string;
          localWorkspacePath: string;
        }) => Promise<ArcTeamPullResult | { error: string }>;
        syncWorkspace: (params: {
          workspaceId: string;
          localWorkspacePath: string;
        }) => Promise<ArcTeamSyncResult | { error: string }>;
        deleteCapsule: (params: {
          workspaceId: string;
          capsuleId: string;
        }) => Promise<ArcTeamDeleteResult | { error: string }>;
        getProjectAutoShare: (params: {
          workspaceId: string;
          localWorkspacePath: string;
        }) => Promise<{ enabled?: boolean; error?: string }>;
        setProjectAutoShare: (params: {
          workspaceId: string;
          localWorkspacePath: string;
          enabled: boolean;
        }) => Promise<{ enabled?: boolean; error?: string }>;
        listMembers: (workspaceId: string) => Promise<ArcTeamMember[] | { error: string }>;
        listPendingInvites: (workspaceId: string) => Promise<ArcTeamPendingInvite[] | { error: string }>;
        listJoinableInvites: () => Promise<ArcTeamJoinableInvite[] | { error: string }>;
        wrapPendingKeys: (workspaceId: string) => Promise<IpcResult & { wrapped?: number }>;
        initializeWorkspaceAccess: (workspaceId: string) => Promise<ArcTeamStatus | { error: string }>;
        setMemberRole: (params: {
          workspaceId: string;
          userId: string;
          role: Exclude<ArcTeamRole, "owner">;
        }) => Promise<ArcTeamStatus | { error: string }>;
        ...
        createInvite: (params: {
          ...
          email?: string;
          role?: ArcTeamRole;
        }) => Promise<ArcTeamInviteResult | { error: string }>;
        acceptInvite: (token: string) => Promise<ArcTeamStatus | { error: string }>;
        acceptInviteById: (inviteId: string) => Promise<ArcTeamStatus | { error: string }>;
      };
      jira: {
