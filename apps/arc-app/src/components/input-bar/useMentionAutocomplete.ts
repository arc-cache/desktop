...
function createMentionIcon(isDir: boolean): SVGSVGElement {
  ...
  svg.setAttribute("class", isDir ? "h-3 w-3 shrink-0 text-blue-400" : "h-3 w-3 shrink-0 text-muted-foreground");

  if (isDir) {
    svg.append(createSvgPath("M20 20a2 2 0 0 0 2-2V8a2 2 0 0 0-2-2h-7.9a2 2 0 0 1-1.69-.9L9.6 3.9A2 2 0 0 0 7.93 3H4a2 2 0 0 0-2 2v13a2 2 0 0 0 2 2Z"));
    return svg;
  }

  svg.append(
    createSvgPath("M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z"),
    createSvgPath("M14 2v4a2 2 0 0 0 2 2h4"),
  );
  return svg;
}
...
export function useMentionAutocomplete({
  ...
  const selectMention = useCallback(
    (entry: MentionEntry) => {
      ...
      label.textContent = `${isDeepDir ? "#" : ""}${entry.path}`;
      chip.append(createMentionIcon(entry.isDir), label);
