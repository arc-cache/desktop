...
import {
  ...
  type CopilotStatusView,
} from "./EnginePickerDropdown";
...
type CopilotAuthStatusResponse = {
  account?: unknown;
...
const COPILOT_LOGIN_COMMAND = "copilot\n/login";
const COPILOT_INSTALL_AND_LOGIN_COMMAND =
  "npm install -g @github/copilot\ncopilot\n/login";

function isCopilotQuotaText(message: string): boolean {
...
function accountDisplayName(account: unknown): string | null {
  if (!account || typeof account !== "object") return null;
  const record = account as Record<string, unknown>;
  for (const key of ["label", "login", "email", "username", "name"]) {
...
function copilotStatusFromResponse(
  response: CopilotAuthStatusResponse,
): CopilotStatusView {
  if (response.authMode === "ollama") {
    return {
      kind: "signed-in",
      label: "Using Ollama provider",
      detail: "Copilot auth is not required for this provider.",
    };
  }

  const status = response.copilotAuthStatus;
  const statusText = [
    response.quotaMessage,
    response.authError,
    response.authGuidance,
    status?.statusMessage,
  ].filter(Boolean).join("\n");

  if (response.quotaExceeded || isCopilotQuotaText(statusText)) {
    return {
      kind: "quota-exceeded",
      label: "Copilot quota exhausted",
      detail: "Signed in, but this Copilot account is out of chat requests for the current period.",
    };
  }

  if (/enoent|not found|no such file|spawn .*copilot/i.test(statusText)) {
    return {
      kind: "runtime-missing",
      label: "Copilot CLI not found",
      detail: "Install @github/copilot, then log in from the OS account that launches ARC.",
      loginCommand: COPILOT_INSTALL_AND_LOGIN_COMMAND,
    };
  }

  if (response.requiresOpenaiAuth || status?.isAuthenticated === false) {
    return {
      kind: "signed-out",
      label: "Copilot is not signed in",
      detail: "Run copilot, then type /login in that terminal.",
      loginCommand: COPILOT_LOGIN_COMMAND,
    };
  }
  ...
  if (response.authError) {
    return {
      kind: "error",
      label: "Could not check Copilot",
      detail: response.authError,
      loginCommand: COPILOT_LOGIN_COMMAND,
    };
  }

  const label = accountDisplayName(response.account) ?? status?.login;
  const detail = [status?.host, status?.authType].filter(Boolean).join(" · ");
  return {
    kind: "signed-in",
    label: label ? `Signed in as ${label}` : "Signed in to Copilot",
    ...(detail ? { detail } : {}),
  };
}
...
export const InputBar = memo(function InputBar({
  ...
  const copilotStatusRequestRef = useRef(0);

  // ── Derived engine state ──
  const isACPAgent = selectedAgent != null && selectedAgent.engine === "acp";
  const isCodexAgent = selectedAgent != null && (selectedAgent.engine === "codex" || selectedAgent.engine === "copilot");
  const isCopilotAgent = selectedAgent != null && selectedAgent.engine === "copilot";
  ...
  const [copilotStatus, setCopilotStatus] = useState<CopilotStatusView>({
    kind: "idle",
    label: "",
  });
  ...
  const refreshCopilotStatus = useCallback(async () => {
    if (!isCopilotAgent) {
      setCopilotStatus({ kind: "idle", label: "" });
      return;
    }

    const requestId = copilotStatusRequestRef.current + 1;
    copilotStatusRequestRef.current = requestId;
    setCopilotStatus({
      kind: "checking",
      label: "Checking Copilot sign-in...",
    });

    try {
      const response = await window.claude.copilot.authStatus();
      if (copilotStatusRequestRef.current !== requestId) return;
      setCopilotStatus(
        copilotStatusFromResponse(response as CopilotAuthStatusResponse),
      );
    } catch (error) {
      if (copilotStatusRequestRef.current !== requestId) return;
      setCopilotStatus({
  ...
  }, [isCopilotAgent, refreshCopilotStatus]);

  const copyCopilotLoginCommand = useCallback((command: string) => {
    void window.claude.writeClipboardText(command);
  }, []);
  ...
  const modelsLoadingText = isCodexAgent
    ? (codexModelsLoadingMessage?.trim() || `Loading ${selectedAgent?.engine === "copilot" ? "Copilot" : "Codex"} models...`)
    : "Loading models...";
        ...
        <div className="mx-4 flex items-center gap-1.5 border-t border-border/[0.08] px-1 pt-2 pb-2.5">
          ...
          <div className="flex min-w-0 flex-1 items-center gap-0.5 overflow-x-auto scrollbar-none">
            ...
            <EnginePickerDropdown
              ...
              copilotStatus={isCopilotAgent ? copilotStatus : undefined}
              onRefreshCopilotStatus={refreshCopilotStatus}
              onCopyCopilotLoginCommand={copyCopilotLoginCommand}
            />
