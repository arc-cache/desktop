...
export function AppLayout() {
  ...
  // ARC: land directly in the repo `arc start` was launched from with a ready
  // native Copilot draft — no setup screens, mirroring the old workbench.
  const arcAutoLanded = useRef(false);
  useEffect(() => {
    if (arcAutoLanded.current) return;
    const initialPath = (window.claude as { arcInitialProjectPath?: string | null }).arcInitialProjectPath;
    if (!initialPath) return;
    if (manager.activeSessionId || manager.isDraft) {
      arcAutoLanded.current = true;
      return;
    }
    const project = projectManager.projects.find((entry) => entry.path === initialPath);
    const arcAgent = agents.find((agent) => agent.id === "copilot");
    if (!project || !arcAgent || selectedAgent?.id !== arcAgent.id) return;
    arcAutoLanded.current = true;
    void handleNewChat(project.id);
  }, [projectManager.projects, agents, selectedAgent, manager.activeSessionId, manager.isDraft, handleNewChat]);
    ...
    >
      ...
      <AppSidebar
        ...
        projectActions={{
          ...
          onResetProjectCache: projectManager.resetProjectCache,
