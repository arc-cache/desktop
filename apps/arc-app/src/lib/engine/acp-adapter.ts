...
function providerOptionLabel(option: ACPPermissionEvent["options"][number]): string {
  ...
  switch (option.kind) {
    ...
    default: return option.kind.replace(/_/g, " ");
  }
}

export function providerOptionsFromAcp(options: ACPPermissionEvent["options"]): ProviderPermissionOption[] {
  return options
    .filter((option) => option.optionId && option.kind)
    .map((option) => ({
      id: option.optionId,
...
export function permissionRequestFromAcp(data: ACPPermissionEvent): PermissionRequest {
  return {
    ...
    providerOptions: providerOptionsFromAcp(data.options),
  };
}

export function optionIdForAcpPermissionBehavior(
  options: ACPPermissionEvent["options"],
  behavior: AppPermissionBehavior,
  providerOptionId?: string,
): string | null {
  if (providerOptionId) {
    return options.some((option) => option.optionId === providerOptionId)
      ? providerOptionId
      : null;
  }
  if (behavior === "allow") {
  ...
  if (behavior === "allowForSession") {
    return options.find((option) => option.kind === "allow_always")?.optionId ?? null;
  }
