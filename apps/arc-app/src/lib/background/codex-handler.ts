...
import type { PermissionRequest, ToolUseResult } from "@/types";
...
export function handleCodexEvent(
  ...
  switch (method) {
    case "arc/memory":
      ...
      state.messages.push(createSystemMessage(`${params.title}: ${params.text}`));
      break;
    ...
    case "turn/completed":
      ...
      finalizePendingCodexToolCalls(state, "Turn ended before this tool returned.");
...
function interruptedToolResult(previous: ToolUseResult | undefined, message: string): ToolUseResult {
  ...
  return {
    ...
    stdout: existingText ? `${existingText}\n${message}` : message,
    status: "failed",
  };
}

export function finalizePendingCodexToolCalls(state: InternalState, message: string): number {
  const pendingMessageIds = new Set(state.parentToolMap.values());
  if (pendingMessageIds.size === 0) return 0;

  let finalized = 0;
  for (const msg of state.messages) {
    if (msg.role !== "tool_call" || !pendingMessageIds.has(msg.id)) continue;
    msg.toolResult = interruptedToolResult(msg.toolResult, message);
    msg.toolError = true;
    finalized++;
  }
  state.parentToolMap.clear();
  return finalized;
}
