...
describe("codex web search mapping", () => {
  ...
  it("closes pending background tool cards when a turn completes without item completion", () => {
    const state = createState();
    ...
    expect(state.messages[0]).toMatchObject({
      ...
      toolResult: {
        type: "text",
        status: "failed",
        stdout: "Turn ended before this tool returned.",
      },
    });
    expect(state.parentToolMap.size).toBe(0);
  });
});
