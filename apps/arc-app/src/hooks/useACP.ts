...
export function useACP({ sessionId, initialMessages, initialConfigOptions, initialSlashCommands, initialMeta, initialPermission, initialRawAcpPermission, acpPermissionBehavior }: UseACPOptions) {
  ...
  const handleSessionUpdate = useCallback((event: ACPSessionEvent) => {
    ...
    if (kind === "agent_message_chunk" || kind === "agent_thought_chunk") {
      ...
      if (content?.type === "text" && content.text) {
        // ARC memory notices are status lines from the arc acp middleware, not
        // model thinking. They can arrive after the turn ended (the review runs
        // post-turn), where a thought chunk would render as a dangling
        // "Thinking…" block — show them as quiet system notes instead.
        if (kind === "agent_thought_chunk" && content.text.startsWith("ARC memory")) {
          setMessages(prev => [...prev, createSystemMessage(content.text.trim())]);
          return;
        }
      ...
      if (autoOptionId) {
        ...
        void window.claude.acp.respondPermission(data._sessionId, data.requestId, autoOptionId)
          .then((result) => {
            ...
            setPendingPermission(permissionRequestFromAcp(data));
          })
          .catch((err) => {
  ...
  const respondPermission = useCallback(async (
    ...
    const optionId = optionIdForAcpPermissionBehavior(acpData.options, behavior, providerOptionId);
    ...
    if (!optionId) {
      toast.error("Failed to respond to permission prompt", {
        description: providerOptionId
          ? "That permission option is no longer available."
          : "No matching ACP permission option was available.",
      });
      return;
    }
  ...
  const setConfig = useCallback(async (configId: string, value: string) => {
    ...
    if (!option) {
      toast.error("Cannot update agent setting", {
        description: `The current agent did not expose "${configId}".`,
      });
      return;
    }
    const values = flattenConfigOptions(option.options).map((entry) => entry.value);
    if (!values.includes(value)) {
      toast.error("Cannot update agent setting", {
        description: `"${value}" is not available for ${option.name}.`,
      });
      return;
    }
    ...
    if (result.configOptions) {
      ...
      setConfigOptions((prev) => prev.map((entry) =>
