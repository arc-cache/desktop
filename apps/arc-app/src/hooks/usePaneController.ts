...
export function usePaneController(
  ...
  return useMemo(() => {
      ...
      : session?.agentId
        ...
        : session?.engine === "codex"
          ...
          : session?.engine === "copilot"
            ? ctx.agents.find((agent) => agent.engine === "copilot") ?? null
          : null;
    ...
    const paneCodexLikeHook = paneEngine === "copilot" ? paneState.copilot : paneState.codex;
    ...
    const paneSupportedModels = paneEngine === "acp"
      ? []
      : paneEngine === "codex" || paneEngine === "copilot"
        ? (paneCodexLikeHook.codexModels.length > 0
          ? paneCodexLikeHook.codexModels
    ...
    const paneAcpConfigOptionsLoading = paneEngine === "acp"
      ...
      : false;
    const paneCodexModelsLoadingMessage = (paneEngine === "codex" || paneEngine === "copilot") && paneSupportedModels.length === 0
    ...
    const handlePaneCodexEffortChange = (effort: string) => {
      ...
      paneCodexLikeHook.setCodexEffort(effort);
    };
    ...
    const handlePaneSend = async (text: string, images?: ImageAttachment[], displayText?: string) => {
      ...
      if (paneEngine === "copilot") {
        try {
          const sent = await paneState.copilot.send(text, images, displayText);
          if (!sent) {
            await ctx.queueSplitPaneSendAfterSwitch?.(sessionId, text, images, displayText);
          }
        } catch (err) {
    ...
    return {
      ...
      paneCodexEffort: isActiveSessionPane ? ctx.manager.codexEffort : paneCodexLikeHook.codexEffort,
