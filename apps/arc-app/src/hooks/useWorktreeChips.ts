...
export const WORKTREE_SETUP_PATH = ".arc/worktree.json";
export const LEGACY_WORKTREE_SETUP_PATH = ".harnss/worktree.json";
...
export function useWorktreeChips(projectPath: string | undefined) {
  ...
  const fetchWorktrees = useCallback(async (scopePath: string) => {
    ...
    try {
      const [repos, setupResult, legacySetupResult] = await Promise.all([
        discoverReposCached(scopePath),
        window.claude.readFile(`${scopePath}/${WORKTREE_SETUP_PATH}`),
        window.claude.readFile(`${scopePath}/${LEGACY_WORKTREE_SETUP_PATH}`),
      ]);
      ...
      setHasSetupFile(
        (!setupResult.error && !!setupResult.content) ||
        (!legacySetupResult.error && !!legacySetupResult.content),
      );
