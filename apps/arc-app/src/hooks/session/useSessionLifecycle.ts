...
interface UseSessionLifecycleParams {
  ...
  prefetchCodexModels: (preferredModel?: string, engine?: "codex" | "copilot") => Promise<void>;
  ...
  reviveCopilotSession: (text: string, images?: ImageAttachment[], displayText?: string) => Promise<void>;
  // From message queue
  ...
  const send = useCallback(
    async (text: string, images?: ImageAttachment[], displayText?: string) => {
      ...
      if (activeId === DRAFT_ID) {
        ...
        if (draftEngine === "copilot") {
          trackMessageSent();
          const sessionId = await materializeDraft(text, images, displayText);
          if (!sessionId) return;
          await new Promise((resolve) => setTimeout(resolve, 50));

          copilot.setMessages((prev) => [
            ...prev,
            createUserMessage(text, images, displayText),
          ]);
      ...
      if (activeSessionEngine === "copilot") {
        if (refs.liveSessionIdsRef.current.has(activeId)) {
          await copilot.send(text, images, displayText);
          return;
        }
        await reviveCopilotSession(text, images, displayText);
        return;
      }
