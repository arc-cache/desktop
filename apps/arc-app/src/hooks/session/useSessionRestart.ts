...
export function useSessionRestart({
  ...
  const restartActiveSessionInCurrentWorktree = useCallback(async (): Promise<{ ok?: boolean; error?: string }> => {
    ...
    if (session.engine === "copilot") {
      ...
      if (!copilotThreadId) {
        return { error: "Copilot session cannot be restarted in another worktree because no thread ID is available." };
      }

      const resumeResult = await window.claude.copilot.resume({
        cwd: nextCwd,
      ...
      if (resumeResult.error || !resumeResult.sessionId) {
        return { error: resumeResult.error || "Failed to restart Copilot session in the selected worktree." };
      }
      ...
      setSessions((prev) => prev.map((s) =>
        s.id === currentId
          ? { ...s, id: newId, copilotThreadId: resumeResult.threadId ?? copilotThreadId }
          : s,
      ));
      setInitialMessages(messagesRef.current);
      setInitialMeta({
      ...
      suppressNextSessionCompletion(currentId);
      await window.claude.copilot.stop(currentId);
      liveSessionIdsRef.current.delete(currentId);
      backgroundStoreRef.current.delete(currentId);
      return { ok: true };
    }
