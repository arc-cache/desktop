...
export interface UseSessionPaneOptions {
  ...
  copilotSessionModel: string | undefined;
...
export interface SessionPaneState {
  ...
  engine: ReturnType<typeof useClaude> | ReturnType<typeof useACP> | ReturnType<typeof useCodex> | ReturnType<typeof useCopilot>;
...
export function useSessionPane({
  ...
  const copilot = useCopilot({
    ...
    sessionModel: copilotSessionModel,
    initialMessages: isCopilot ? initialMessages : [],
    initialMeta: isCopilot ? initialMeta : null,
    initialPermission: isCopilot ? initialPermission : null,
  });

  // Pick the active engine's state
  const engine = isCopilot ? copilot : isCodex ? codex : isACP ? acp : claude;
