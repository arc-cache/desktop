...
export function useSessionRevival({
  ...
  const { acp, codex, copilot, engine } = engines;
  const {
    setSessions,
    setActiveSessionId,
    setInitialMessages,
  ...
  /** Revive a dead Copilot session — spawn new ARC app-server + thread/resume */
  const reviveCopilotSession = useCallback(
    async (text: string, images?: ImageAttachment[], displayText?: string) => {
      const oldId = activeSessionIdRef.current;
      if (!oldId || oldId === DRAFT_ID) return;
      const session = sessionsRef.current.find((s) => s.id === oldId);
      ...
      let copilotThreadId: string | undefined = session.copilotThreadId;
      if (!copilotThreadId) {
        try {
          const persisted = await window.claude.sessions.load(session.projectId, oldId);
          copilotThreadId = persisted?.copilotThreadId;
        } catch { /* ignore */ }
      }

      if (!copilotThreadId) {
        copilot.setMessages((prev) => [...prev, createSystemMessage("Copilot session cannot be resumed (no thread ID). Please start a new session.", true)]);
        return;
      }

      const result = await window.claude.copilot.resume({
        cwd: getProjectCwd(project),
      ...
      if (result.error || !result.sessionId) {
        copilot.setMessages((prev) => [...prev, createSystemMessage(result.error || "Failed to resume Copilot session.", true)]);
        return;
      }
      ...
      setSessions((prev) => prev.map((s) =>
        s.id === oldId ? { ...s, id: newId, copilotThreadId: result.threadId ?? copilotThreadId } : s,
      ));
      setInitialMessages(messagesRef.current);
      setInitialMeta({
        isProcessing: false,
      ...
      await new Promise((resolve) => setTimeout(resolve, 50));
      copilot.setMessages((prev) => [...prev, createUserMessage(text, images, displayText)]);
      copilot.setIsProcessing(true);
      capture("message_sent", {
        engine: "copilot",
        has_images: !!images?.length,
        message_length: text.length,
      });
      const sendResult = await window.claude.copilot.send(
        newId,
        text,
        imageAttachmentsToCodexInputs(images),
        codexEffortRef.current,
      ...
      if (sendResult?.error) {
        copilot.setMessages((prev) => [...prev, createSystemMessage(`Unable to send message: ${sendResult.error}`, true)]);
        copilot.setIsProcessing(false);
      }
    },
    [findProject, copilot.setMessages, copilot.setIsProcessing],
  );
