...
export function useDraftMaterialization({
  ...
  const { claude, acp, codex, copilot } = engines;
  const {
    setSessions,
    setActiveSessionId,
    setInitialMessages,
  ...
  const prefetchCodexModels = useCallback(async (preferredModel?: string, targetEngine: "codex" | "copilot" = "codex") => {
    const api = targetEngine === "copilot" ? window.claude.copilot : window.claude.codex;
    const targetHook = targetEngine === "copilot" ? copilot : codex;
    const setRawModels = targetEngine === "copilot" ? setCopilotRawModels : setCodexRawModels;
    setCodexModelsLoadingMessage(targetEngine === "copilot" ? "Checking Copilot SDK..." : "Checking Codex CLI...");
    try {
      const status = await api.binaryStatus();
      if (!status.installed) {
        setCodexModelsLoadingMessage(targetEngine === "copilot" ? "Copilot CLI not found." : "Codex CLI not found. Downloading it now...");
      }

      const result = await api.listModels();
      if (result.error) {
        setCodexModelsLoadingMessage(`${targetEngine === "copilot" ? "Copilot" : "Codex"} model load failed: ${result.error}`);
        return;
      }
      const models = normalizeCodexModels(result.models ?? []);
      if (models.length === 0) {
        setCodexModelsLoadingMessage(`No ${targetEngine === "copilot" ? "Copilot" : "Codex"} models available yet.`);
        return;
      }

      setRawModels(models);
      targetHook.setCodexModels(models.map((m) => ({
        value: m.id,
      ...
      setStartOptions((prev) => {
        if ((prev.engine ?? "claude") !== targetEngine) return prev;
      ...
      setCodexModelsLoadingMessage(`Failed to initialize ${targetEngine === "copilot" ? "Copilot" : "Codex"}: ${message}`);
    }
  }, [applyCodexModelDefaultEffort, codex, copilot, setCodexModelsLoadingMessage, setCodexRawModels, setCopilotRawModels, setStartOptions]);
  ...
  const materializeDraft = useCallback(
    async (text: string, images?: ImageAttachment[], displayText?: string) => {
      ...
      let copilotThreadId: string | undefined;
      ...
      if (draftEngine === "acp" && options.agentId) {
      ...
      } else if (draftEngine === "codex" || draftEngine === "copilot") {
        // Codex/Copilot app-server path
        const targetApi = draftEngine === "copilot" ? window.claude.copilot : window.claude.codex;
        const targetHook = draftEngine === "copilot" ? copilot : codex;
        setSessions(prev => [{
          id: DRAFT_ID,
          projectId: project.id,
          title: "New Chat",
          ...
          isActive: true,
          engine: draftEngine,
          agentId: options.agentId ?? draftEngine,
        }, ...prev.map(s => ({ ...s, isActive: false }))]);

        const rawModels = draftEngine === "copilot" ? copilotRawModelsRef.current : codexRawModelsRef.current;
        const setRawModels = draftEngine === "copilot" ? setCopilotRawModels : setCodexRawModels;
        const requestedDraftModel = options.model?.trim();
        const draftModel = rawModels.length > 0
          ? pickCodexModel(options.model, rawModels)
          : requestedDraftModel || undefined;
        const approvalPolicy = getCodexApprovalPolicy(options);
        const sandbox = getCodexSandboxMode(options);
        const result = await targetApi.start({
          cwd: getProjectCwd(project),
        ...
        if (result.error || !result.sessionId) {
          const errorMsg = result.error || `Failed to start ${draftEngine === "copilot" ? "Copilot" : "Codex"} session`;
          const failedId = `failed-${draftEngine}-${Date.now()}`;
          const errorMessages: UIMessage[] = [
            createUserMessage(text, images, displayText),
            createSystemMessage(errorMsg, true),
          ];
        ...
        sessionId = result.sessionId;
        if (draftEngine === "copilot") copilotThreadId = result.threadId;
        else codexThreadId = result.threadId;
      ...
      const newSession: ChatSession = {
        ...
        ...(draftEngine === "copilot" ? {
          agentId: options.agentId ?? "copilot",
          copilotThreadId,
        } : {}),
      };
    ...
    [acp, applyCodexModelDefaultEffort, findProject, generateSessionTitle, codex.setCodexModels, copilot.setCodexModels, setDraftAcpSessionId],
  );
