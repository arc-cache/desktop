...
export function useSessionCache({
      ...
      setSessions((prev) =>
        prev.filter((s) => s.id !== DRAFT_ID).map((s) => ({
          ...
          ...(s.id === id ? {
            ...
            ...(data.copilotThreadId ? { copilotThreadId: data.copilotThreadId } : {}),
  ...
  // Ensure Codex/Copilot model metadata is available even before first turn.
  useEffect(() => {
    if (activeEngine !== "codex" && activeEngine !== "copilot") return;
    const targetModels = activeEngine === "copilot" ? copilot.codexModels : codex.codexModels;
    ...
    prefetchCodexModels(preferredModel, activeEngine);
  }, [
    activeEngine,
    activeSessionId,
