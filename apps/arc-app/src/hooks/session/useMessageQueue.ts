...
export function useMessageQueue({ refs, setters, engines, activeSessionId }: UseMessageQueueParams) {
  ...
  const updateSessionMessages = useCallback((
    sessionId: string,
    sessionEngine: "claude" | "acp" | "codex" | "copilot",
    updater: (prev: UIMessage[]) => UIMessage[],
  ) => {
    if (sessionId === activeSessionIdRef.current) {
      const targetSetMessages = sessionEngine === "copilot"
        ? copilot.setMessages
        : sessionEngine === "codex"
        ? codex.setMessages
        : sessionEngine === "acp"
  ...
  }, [activeSessionIdRef, acp.setMessages, backgroundStoreRef, claude.setMessages, codex.setMessages, copilot.setMessages]);
  ...
  const setSessionProcessing = useCallback((
    ...
    if (sessionId === activeSessionIdRef.current) {
      const targetSetIsProcessing = sessionEngine === "copilot"
        ? copilot.setIsProcessing
        : sessionEngine === "codex"
  ...
  }, [activeSessionIdRef, acp.setIsProcessing, backgroundStoreRef, claude.setIsProcessing, codex.setIsProcessing, copilot.setIsProcessing]);
  ...
  const drainQueuedMessageForSession = useCallback(async (sessionId: string) => {
    ...
    try {
      if (sessionEngine === "acp") {
        ...
        if (result?.error) handleSendError("Failed to send queued message.");
      } else if (sessionEngine === "copilot") {
        setSessionProcessing(sessionId, sessionEngine, true);
        const result = await window.claude.copilot.send(
          sessionId,
          next.text,
          imageAttachmentsToCodexInputs(next.images),
          codexEffortRef.current,
  ...
  useEffect(() => {
    ...
    if (sessionEngine === "acp") {
      ...
      void window.claude.copilot.interrupt(activeId);
    } else {
