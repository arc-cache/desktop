...
import { permissionRequestFromAcp, pickAutoResponseOption } from "../../lib/engine/acp-adapter";
...
export function useSessionPersistence({
  ...
  const { claude, acp, codex, copilot, engine } = engines;
  ...
  // Persist session with native thread ID fallback
  const persistSessionWithCodexFallback = useCallback(async (data: PersistedSession) => {
    ...
    if (data.engine === "copilot" && !data.copilotThreadId) {
      try {
        const existing = await window.claude.sessions.load(data.projectId, data.id);
        if (existing?.copilotThreadId) payload = { ...payload, copilotThreadId: existing.copilotThreadId };
      } catch {
        // Best-effort fallback only.
      }
    }
  ...
  useEffect(() => {
    ...
    const unsubCopilotExit = window.claude.copilot.onExit((data) => handleSessionExit(data._sessionId));
    return () => {
      unsubExit();
      unsubAcpExit();
      unsubCodexExit();
  ...
  useEffect(() => {
    ...
    const unsubCopilot = window.claude.copilot.onEvent((event) => {
      const sid = event._sessionId;
      if (!sid || sid === activeSessionIdRef.current || visibleSplitSessionIdsRef.current.includes(sid)) return;
      backgroundStoreRef.current.handleCodexEvent(event);
    });

    // Route native approval requests for non-active sessions into the background store.
    const unsubCodexApproval = window.claude.codex.onApprovalRequest((data) => {
      const sid = data._sessionId;
      if (!sid || sid === activeSessionIdRef.current || visibleSplitSessionIdsRef.current.includes(sid)) return;
      backgroundStoreRef.current.setPermission(
        sid,
        permissionRequestFromCodexServerRequest(data, "codex_request_user_input"),
      );
    });
    const unsubCopilotApproval = window.claude.copilot.onApprovalRequest((data) => {
      const sid = data._sessionId;
      if (!sid || sid === activeSessionIdRef.current || visibleSplitSessionIdsRef.current.includes(sid)) return;
      backgroundStoreRef.current.setPermission(
        sid,
        permissionRequestFromCodexServerRequest(data, "copilot_request_user_input"),
      );
    });

    return () => { unsub(); unsubAcp(); unsubBgPerm(); unsubBgAcpPerm(); unsubBgAcpTurn(); unsubCodex(); unsubCopilot(); unsubCodexApproval(); unsubCopilotApproval(); };
  }, []);
  ...
  useEffect(() => {
    ...
    saveTimerRef.current = setTimeout(() => {
      ...
      const data: PersistedSession = {
        ...
        ...(session.engine === "copilot" && session.copilotThreadId ? { copilotThreadId: session.copilotThreadId } : {}),
      };
  ...
  }, [claude.slashCommands, acp.slashCommands, codex.slashCommands, copilot.slashCommands]);
    ...
    async (sessionId: string, message: string, projectPath: string, titleEngine?: EngineId) => {
      ...
      try {
        ...
        const result = await window.claude.generateTitle(
          ...
          projectPath,
          titleEngine,
          titleEngine && titleEngine !== "claude" ? sessionId : undefined,
        );
