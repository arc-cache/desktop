...
export function useAppOrchestrator() {
  ...
  // ARC: default new chats to the native Copilot engine.
  const arcDefaultApplied = useRef(false);
  useEffect(() => {
    if (arcDefaultApplied.current || selectedAgent) return;
    const arcAgent = agents.find((agent) => agent.id === "copilot");
    if (arcAgent) {
      arcDefaultApplied.current = true;
      setSelectedAgent(arcAgent);
    }
  ...
  useEffect(() => {
    ...
    if (session.engine === "copilot") {
      const copilotAgent = (session.agentId
        ? agents.find((a) => a.id === session.agentId)
        : undefined) ?? agents.find((a) => a.engine === "copilot");
      if (copilotAgent && selectedAgent?.id !== copilotAgent.id) {
        setSelectedAgent(copilotAgent);
      }
      return;
    }
