...
export function useSessionManager(
  ...
  const [copilotRawModels, setCopilotRawModels] = useState<CodexModelSummary[]>([]);
  ...
  const isCopilot = activeEngine === "copilot";
  ...
  const copilotSessionId = (activeEngine === "copilot" && activeSessionId !== DRAFT_ID) ? activeSessionId : null;
  const codexSessionModel = (activeEngine === "codex" && activeSessionId !== DRAFT_ID)
    ...
    : undefined;
  const copilotSessionModel = (activeEngine === "copilot" && activeSessionId !== DRAFT_ID)
    ? (sessions.find((s) => s.id === activeSessionId)?.model ?? startOptions.model)
    : undefined;
  ...
  const { claude, acp, codex, copilot, engine } = primaryPane;
  ...
  const copilotRawModelsRef = useRef(copilotRawModels);
  copilotRawModelsRef.current = copilotRawModels;
  const codexEffortRef = useRef(isCopilot ? copilot.codexEffort : codex.codexEffort);
  codexEffortRef.current = isCopilot ? copilot.codexEffort : codex.codexEffort;
  ...
  const setCodexEffortFromUser = useCallback((effort: string) => {
    ...
    if (isCopilot) copilot.setCodexEffort(effort);
    else codex.setCodexEffort(effort);
  }, [codex.setCodexEffort, copilot.setCodexEffort, isCopilot]);
  const applyCodexModelDefaultEffort = useCallback((effort: string | undefined) => {
    ...
    copilot.setCodexEffort(effort);
  }, [codex.setCodexEffort, copilot.setCodexEffort]);
  ...
  const { reviveSession, reviveAcpSession, reviveCodexSession, reviveCopilotSession } = useSessionRevival({
    refs,
    setters,
    engines,
    findProject,
  ...
  return {
    ...
    mcpServerStatuses: isACP || isCodex || isCopilot
    ...
    mcpStatusPreliminary: isDraft && draftMcpStatuses.length > 0 && (
      isACP || isCodex || isCopilot ? acpMcpStatuses.length === 0 : claude.mcpServerStatuses.length === 0
    ),
    refreshMcpStatus: isACP || isCodex || isCopilot
      ? (() => Promise.resolve())
    ...
    reconnectMcpServer: isACP
      ...
      : isCodex || isCopilot
        ? async (_name: string) => { /* Codex/Copilot MCP reconnect: not yet implemented */ }
      : (preStartedSessionId && isDraft)
        ? (async (name: string) => {
    ...
    restartWithMcpServers: isACP
      ...
      : isCodex || isCopilot
        ? async (_servers: McpServerConfig[]) => { /* Codex/Copilot MCP restart: not yet implemented */ }
      : (preStartedSessionId && isDraft)
    ...
    codexEffort: isCopilot ? copilot.codexEffort : codex.codexEffort,
    ...
    completeAcpAuth,
    cancelAcpAuth,
    codexRawModels: isCopilot ? copilotRawModels : codexRawModels,
    codexModelsLoadingMessage,
    // Codex plan steps (from turn/plan/updated events — separate from Claude's TodoWrite tool)
    codexTodoItems: isCopilot ? copilot.todoItems : codex.todoItems,
  };
}
